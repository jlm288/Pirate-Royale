...
const SHRINK_INTERVAL = 30000; // 30s between shrinks
...
for (const [id, p] of Object.entries(players)) {
  ...
  if (Math.sqrt(dx * dx + dy * dy) < SHIP_RADIUS) {
    ...
    if (p.hp <= 0) {
      ...
      io.emit('playerDied', { id, killedBy: shooter?.name || 'The Sea' });
      checkWin();
    }
    return false;
  }
...
socket.on(‘join’, ({ name }) => {
const cleanName = (name || ‘’).trim().slice(0, 16) || PIRATE_NAMES[Math.floor(Math.random() * PIRATE_NAMES.length)];
const pos = randomSpawn();
...
socket.emit('joined', { id: socket.id, player: players[socket.id], worldW: WORLD_W, worldH: WORLD_H });
...
p.reloadReady = false;
bullets.push({ id: bulletId++, x: p.x, y: p.y, angle, ownerId: socket.id, life: 120 });
io.emit(‘fired’, { ownerId: socket.id, x: p.x, y: p.y });
setTimeout(() => { if (players[socket.id]) players[socket.id].reloadReady = true; }, RELOAD_TIME);
});
...
server.listen(PORT, () => {
// Works locally and on Railway/Render/any cloud host
